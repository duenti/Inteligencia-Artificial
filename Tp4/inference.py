...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    # print ("Distancia barulgo: ",  noisyDistance, "\n")
    # print ("Modelo emissão: " , emissionModel)
    # print ("Posição Pacman:  " , pacmanPosition)
  ...
  def elapseTime(self, gameState):
    ...
    for p in self.beliefs:
      if(self.beliefs[p] > 0):
        ...
        proxPosicao = self.getPositionDistribution(gameState)
        for i in self.legalPositions:
          beliefs[i] += self.beliefs[p]*proxPosicao[i]
    beliefs.normalize()

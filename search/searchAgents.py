...
class CornersProblem(search.SearchProblem):
  ...
  def getStartState(self):
    ...
    "Posição,lista com os cantos visitados"
  ...
  def isGoalState(self, state):
    ...
    if no in self.corners:
      if not no in cantosVisitados:
        cantosVisitados.append(no)
      if len(cantosVisitados) == 4:
        return True
    return False
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
        pNo = (nextx,nexty)
        succCanVis = list(cantosVisitados)
        if pNo in self.corners:
          if not pNo in succCanVis:
            succCanVis.append(pNo)
        state = (pNo,succCanVis)
...
def cornersHeuristic(state, problem):
  ...
  melhorDistancia = 999999
  for canto in cantos:
    dist2 = abs(atual[0] - canto[0]) + abs(atual[1] - canto[1])
    if dist2 < melhorDistancia:
...
def foodHeuristic(state, problem):
  ...
  cordList = foodGrid.asList()
  "*** YOUR CODE HERE ***"

  maxDistancia = -1
  for comida in cordList:
    dist2 = abs(position[0] - comida[0]) + abs(position[1] - comida[1])
    if dist2 > maxDistancia:
      maxDistancia = dist2
